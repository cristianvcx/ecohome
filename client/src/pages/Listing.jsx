            ...
            {listing && !loading && !error && (<div>
                ...
                <div className='flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4'>
                    <p className='text-2xl font-semibold'>
                        {listing.name} - ${' '}
                        {listing.offer
                            ? listing.discountPrice.toLocaleString('es-CL')
                            : listing.regularPrice.toLocaleString('es-CL')}
                        ...
                        </p>
                        {
                            listing.offer && (
                                <p className='bg-green-900 w-full max-w-[200px] text-white text-center p-1 rounded-md'>
                                    ${+listing.regularPrice - +listing.discountPrice} OFERTA
                                </p>
                            )
                        }
                    </div>
                    ...
                    <ul className='text-green-900 font-semibold text-sm flex items-center gap-4 sm:gap-6'>
                        <li className='flex items-center gap-1 whitespace-nowrap'>
                            ...
                            {listing.bedrooms > 1 ? `${listing.bedrooms} cama ` : `${listing.bedrooms} camas `}
                        </li>
                        <li className='flex items-center gap-1 whitespace-nowrap'>
                            <FaBath className='text-lg' />
                            {listing.bathrooms > 1 ? `${listing.bathrooms} baño ` : `${listing.bathrooms} baños `}
                        </li>
                        <li className='flex items-center gap-1 whitespace-nowrap'>
                            <FaParking className='text-lg' />
                            {listing.parking ? 'Estacionamiento' : 'No Estacionamiento'}
                        </li>
                        <li className='flex items-center gap-1 whitespace-nowrap'>
                    ...
                    {currentUser && listing.userRef !== currentUser._id && !contact && (
                        <button onClick={()=>setContact(true)} className='bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 p-3'>Contactar al Propietario</button>
                    )}
